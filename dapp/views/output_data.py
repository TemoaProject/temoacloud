# Django
from dapp.models import DataFile, CommTech, Actions, ModelRun, OutputDataRun, InputOutputDataFile, Scenario, OutputPlot
...
def generate_plot(request, project_uid, scenario_uid):
    ...
    criterion2 = Q(account=request.user)
    project = get_object_or_404(Project, criterion1 & criterion2)
    region = request.POST.get("plot-region-value", "")
    ...
    db_path = settings.UPLOADED_PROJECTS_DIR + '{0}/{1}/files/{2}'.format(project_uid, scenario_uid, data_file.name)

    # db_path = settings.UPLOADED_PROJECTS_DIR + filename

    image_path_dir = settings.RESULT_DIR + 'matplot/'
    res = OutputPlotGenerator(db_path, region, scenario)
    mode = "plot"

    plot_path = settings.RESULT_DIR + 'matplot/'
    plot_path_display = settings.RESULT_URL + 'matplot/'
    error = ""
    try:
        account = Account.objects.get(email=request.user.email)
        action_name = scenario_obj.name + '_' + str(str(uuid.uuid4().hex.upper()[0:6]))

        ac = Actions.objects.create(name=action_name, mode=mode, project=project, account=account,
        ...
        output_plot = OutputPlot.objects.create(selected_scenario=scenario,
                                                region=region,
                                                plot_type=plot_type,
                                                sector=sector,
                                                input_file=io_data_file,
                                                scenario=scenario_obj, project=project, account=account,
                                                actions=ac,

        )
        output_plot.save()
        folder_name = os.path.splitext(os.path.basename(db_path))[0] + "_" + region + "_" + scenario + "_plots"
        output_dir = os.path.join(plot_path, folder_name)
        if plot_type == 1:
            plot_path += res.generatePlotForCapacity(sector, super_categories, image_path_dir)
            plot_path_display += res.generatePlotForCapacity(sector, super_categories, image_path_dir)
        elif plot_type == 2:
            plot_path += res.generatePlotForOutputFlow(sector, super_categories, image_path_dir)
            plot_path_display += res.generatePlotForOutputFlow(sector, super_categories, image_path_dir)
        elif plot_type == 3:
            plot_path += res.generatePlotForEmissions(sector, super_categories, image_path_dir)
            plot_path_display += res.generatePlotForEmissions(sector, super_categories, image_path_dir)
        ...
        if os.path.exists(result_download_folder_path + '.zip'):
            shutil.make_archive(result_download_folder_path, 'zip', output_dir)
        shutil.make_archive(result_download_folder_path, 'zip', output_dir)
    ...
    return JsonResponse({"data": plot_path_display, "error": error})
...
def load_sector(request, project_uid, scenario_uid):
    ...
    scenario_obj = get_object_or_404(Scenario, criterion_sc & criterion2)
    project = get_object_or_404(Project, criterion1 & criterion2)
    region = request.GET.get("selected_region", "")
